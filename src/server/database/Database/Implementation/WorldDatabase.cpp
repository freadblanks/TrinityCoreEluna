...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_INS_GAMEOBJECT, "INSERT INTO gameobject (guid, id, map, spawnDifficulties, PhaseId, PhaseGroup, position_x, position_y, position_z, orientation, rotation0, rotation1, rotation2, rotation3, spawntimesecs, animprogress, state, isActive, size, hasDoodads, visibility) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(WORLD_INS_PERMASKYBOX, "INSERT INTO player_skybox (guid, skybox) VALUES (?,?)", CONNECTION_ASYNC);
    PrepareStatement(WORLD_UPD_PERMASKYBOX, "UPDATE player_skybox SET skybox = ? WHERE guid = ?", CONNECTION_ASYNC);
